...
#endif
			if (newsrv->use_ssl) {
				int ssloptions = SSL_OP_NO_SSLv2 | SSL_OP_ALL | SSL_OP_NO_COMPRESSION;
				...
				SSL_CTX_set_mode(newsrv->ssl_ctx.ctx, SSL_MODE_ENABLE_PARTIAL_WRITE|SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER);
				SSL_CTX_set_session_cache_mode(newsrv->ssl_ctx.ctx, SSL_SESS_CACHE_CLIENT);
